...
def test_function_based_error_handling():
    ...
    with patch.dict(os.environ, {"OPENAI_API_KEY": "invalid_key"}):
        ...
        with patch('bespokelabs.curator.request_processor.openai_online_request_processor.OpenAIOnlineRequestProcessor.call_single_request') as mock_call:
    ...
    with patch.dict(os.environ, {"OPENAI_API_KEY": os.getenv("openai_key_3")}):
        with patch('bespokelabs.curator.request_processor.openai_online_request_processor.OpenAIOnlineRequestProcessor.call_single_request') as mock_call:
            mock_call.return_value = MagicMock(
                response_message=TestResponse(message="test response"),
